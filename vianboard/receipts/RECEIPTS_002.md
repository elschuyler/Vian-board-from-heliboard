...
- **Deviation from requested**: None.
- **Known issue or follow-up needed**: Phase 21 Phase A complete; ready for on-device manual QA testing and Phase B (accidental number typo prediction).

---

### Entry 005
- **Timestamp**: 2026-09-13 06:34:00
- **Summary**: Implemented Phase 21 Phase B: Accidental Number Typo Engine and Autocorrect Integration.
- **Requested**: Complete Phase B (accidental number typo detection, letter candidate mappings, and auto-correct veto relaxation).
- **Exact files touched**:
  ...
  - `receipts/RECEIPTS_002.md`
- **What was actually done**:
  1. Updated `WordComposer.java` `getNearbyLetterCandidatesForDigit`:
     - Added standard top-row number position substitutions: `1->q`, `2->w`, `3->e`, `4->r`, `5->t`, `6->y`, `7->u`, `8->i`, `9->o`, `0->p`.
     - Added physical adjacent top-row keys and common leetspeak substitutions:
       - '1' -> 'q', 'i', 'l', 'w', 'a'
       - '2' -> 'w', 'z', 'q', 'e', 's'
       - '3' -> 'e', 'w', 'r', 'd'
       - '4' -> 'r', 'a', 'e', 't', 'f'
       ...
       - '9' -> 'o', 'p', 'g', 'i', 'l'
       - '0' -> 'p', 'o', 'l'
     - Maintained keyboard hint label matching and spatial touch distance calculation when available.
  2. Updated `Suggest.kt` `getSuggestedWordsForNonBatchInput`:
     - When `wordComposer.isAccidentalDigitWord` is true, generates letter substitution candidate words.
     - Evaluates candidates against dictionary facilitator (`mDictionaryFacilitator.isValidSpellingWord`), checking both raw and locale lowercase forms.
     - Adds top valid candidate as high-priority auto-correction target (`SuggestedWordInfo.KIND_WHITELIST` at score `MAX_SCORE - 100`) and secondary candidates as corrections.
     - Preserves user typed digit string at index 0 of the strip with `KIND_TYPED` so the user can easily tap to keep the literal digit if desired.
  3. Relaxed `hasDigits()` auto-correct veto in `Suggest.kt`:
     - Replaced `|| wordComposer.hasDigits()` with `|| (wordComposer.hasDigits() && !wordComposer.isAccidentalDigitWord)`.
  4. Updated `BLUEPRINT.md` marking Phase 21 Phase B [COMPLETED].
- **How it was verified**: Full local Gradle compile verified via `compile_applet` (`BUILD SUCCESSFUL`).
- **Deviation from requested**: None.
...
- **Summary**: Implemented Voice Input View Refactor (Approach B) for system navigation insets and theme key parity.
- **Requested**: Voice input modal going under navigation bar and button styling too different; implement Approach B (compact modal height with dynamic window insets + MainKeyboardView with CLIPBOARD_BOTTOM_ROW for theme key shapes and background).
- **Exact files touched**:
  - `app/src/main/res/layout/voice_input_view.xml`
  - `app/src/main/java/helium314/keyboard/latin/voice/VoiceInputView.kt`
  - `app/src/main/java/helium314/keyboard/keyboard/KeyboardSwitcher.java`
  ...
  - `receipts/RECEIPTS_002.md`
- **What was actually done**:
  1. Updated `voice_input_view.xml`:
     - Set root background to transparent (`@android:color/transparent`).
     - Put top status/streaming row inside weighted container (`layout_height="0dp"`, `layout_weight="1"`, `gravity="center_vertical"`).
     - Replaced custom button LinearLayout and divider with standard `MainKeyboardView` (`id="@+id/bottom_row_keyboard"`).
  2. Refactored `VoiceInputView.kt`:
     - Added `fitsSystemWindows = true` and `orientation = VERTICAL`.
     - Added dynamic navigation bar insets handling (`WindowManager.currentWindowMetrics` and `onApplyWindowInsets`) dynamically adjusting bottom padding by `navBarBottomInset`.
     - In `onMeasure`, ensured compact modal height (`rowHeight + topRowMinHeight + paddingTop + paddingBottom`) cleanly positions the bottom 4 buttons strictly above the Android navigation bar.
     - Extracted `keyBackgroundId` from `KeyboardView` theme attributes to style `gainPill` with active theme key shape, functional key background, and keyboard typeface.
     - Built `setupBottomRowKeyboard` initializing `MainKeyboardView` with `KeyboardElement.CLIPBOARD_BOTTOM_ROW` and `PointerTracker.switchTo()`.
     - Intercepted `onCodeInput` in wrapped listener to stop voice on `KeyCode.ALPHA` (seamless return to alphabet layout) and pause voice on typing actions (`DELETE`, `SPACE`, `ENTER`).
     - Added hardware accelerated drawing delegation.
  3. Updated `KeyboardSwitcher.java`:
     - Passed `mLatinIME.getCurrentInputEditorInfo()` into `mVoiceInputView.startVoiceInput(...)`.
     - Enabled hardware accelerated drawing on `mVoiceInputView` during input view setup.
  4. Updated `BLUEPRINT.md` and `receipts/RECEIPTS_002.md`.
- **How it was verified**: Full local Gradle compile verified cleanly via `compile_applet` (`BUILD SUCCESSFUL`).
- **Deviation from requested**: None.
- **Known issue or follow-up needed**: Ready for on-device manual QA testing.
...
### Entry 007
- **Timestamp**: 2026-09-13 12:15:20
- **Summary**: Implemented Dedicated 2-Line Voice Preview Strip (Approach 1) with 1-second auto-commit buffer and Clear button.
- **Requested**: Implement Approach 1 for voice input: 2-line preview strip in suggestion strip area, 1-second delay before auto-committing text, clear button to restart/cancel transcription, active only during voice input.
- **Exact files touched**:
  - `app/src/main/res/values/strings.xml`
  ...
  - `receipts/RECEIPTS_002.md`
- **What was actually done**:
  1. Added `voice_preview_clear` string resource in `strings.xml`.
  2. Modified `strip_container.xml`: Added `voice_preview_strip` (`LinearLayout`) featuring a 2-line auto-ellipsizing `TextView` (`voice_preview_text`) styled with `12.5sp`, `includeFontPadding="false"`, `lineSpacingMultiplier="0.95"`, and a 44dp `ImageView` (`voice_preview_clear`) utilizing `@drawable/ic_close_rounded` with `selectableItemBackgroundBorderless`.
  3. Modified `KeyboardSwitcher.java`:
     - Registered `mVoicePreviewStrip` field and bound it in `init()`.
     - Ensured `mVoicePreviewStrip` is visible in `setVoiceInputKeyboard()` while keeping `mStripContainer` visible.
     - Ensured `mVoicePreviewStrip` is hidden (`View.GONE`) across alphabet, emoji, clipboard, and prompt layouts.
     - Added public getter `getVoicePreviewStrip()`.
  4. Modified `VoiceInputView.kt`:
     - Acquired references to `previewStrip`, `previewText`, and `previewClear` in `startVoiceInput` and themed them dynamically (`STRIP_BACKGROUND`, `KEY_TEXT`, `KEY_ICON`).
     - Added `scheduleCommit(text)` scheduling auto-commit via `Handler` after a 1000ms delay.
     - Bound clear button click listener to `clearPreview()` which cancels pending commits, resets preview text, and returns status to listening.
     - Added immediate tap-to-commit on `previewText` (`commitPendingImmediately()`).
     - In `onTranscriptionPreview`, displayed interim preview text and canceled prior pending commits if speaking resumed.
     - In `onFinalTranscription`, updated preview and scheduled the 1-second timer.
     - Cleaned up timer callbacks, pending text, and view references in `pauseVoiceInput()` and `stopVoiceInput()`.
  5. Updated `BLUEPRINT.md` and `receipts/RECEIPTS_002.md`.
- **How it was verified**: Full local Gradle compile verified cleanly via `compile_applet` (`BUILD SUCCESSFUL`).
- **Deviation from requested**: None.
- **Known issue or follow-up needed**: Ready for on-device manual QA testing.
