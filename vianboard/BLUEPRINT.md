...
- **Phase 21: Native Suggestion & Gesture Engine**:
  - **Phase A: Library UI Restoration & Dynamic JNI Integration [COMPLETED]**:
  ...
  - **Phase B: Accidental Number Typo Engine [COMPLETED]**:
    - Update `StringUtils.kt` word boundary detection to allow numbers surrounded by script letters (`isPartOfCompositionForScript`).
    - Map digits to physical top-row proximity and leetspeak substitution candidates in `WordComposer.java` and `Suggest.kt`.
    - Relax single-digit `hasDigits()` autocorrect veto in `Suggest.kt` (`wordComposer.hasDigits() && !wordComposer.isAccidentalDigitWord`).
    - Query candidate words against dictionary facilitator, adding top matching candidate as auto-correct target (`KIND_WHITELIST`) with user-typed digit string preserved in suggestion strip.
...
- **2026-09-11**: Completed Phase 21 Phase A (Library UI Restoration & Dynamic JNI Integration): extended `JniUtils.java` with library loader, deletion, and checksum verification APIs; created `LoadGestureLibPreference.kt` with document picker, checksum checking against ABI defaults, confirmation dialogs, and dynamic library linking; integrated into `WordEngineScreen.kt` and `SettingsNavHost.kt`; verified via `compile_applet`.
- **2026-09-13**: Completed Phase 21 Phase B (Accidental Number Typo Engine): relaxed `wordComposer.hasDigits()` autocorrect veto in `Suggest.kt` for `wordComposer.isAccidentalDigitWord`; updated `WordComposer.java` with top-row proximity and leetspeak substitution mappings; integrated candidate dictionary evaluation and prioritized auto-correction injection in `Suggest.kt`; verified clean compilation via `compile_applet`.
- **2026-09-13**: Refactored `VoiceInputView` (Approach B): resolved system navigation bar occlusion by enabling `fitsSystemWindows = true` and dynamically calculating `WindowInsets.Type.navigationBars()`; eliminated custom button LinearLayout in `voice_input_view.xml` and replaced with standard `MainKeyboardView` loaded via `KeyboardElement.CLIPBOARD_BOTTOM_ROW` for complete visual, theme, ripple, and key shape parity with Clipboard and Main Keyboard; styled gain pill with theme key background and functional color; verified build via `compile_applet`.
- **2026-09-13**: Implemented Dedicated 2-Line Voice Preview Strip (Approach 1): added `voice_preview_strip` with 2-line auto-ellipsizing `voice_preview_text` and 44dp `voice_preview_clear` (`[✕]`) into `strip_container.xml`; wired lifecycle visibility in `KeyboardSwitcher.java` so strip only appears during voice input; implemented 1-second auto-commit delay in `VoiceInputView.kt` with immediate tap-to-commit and instant clear-to-cancel; verified clean compilation via `compile_applet`.
