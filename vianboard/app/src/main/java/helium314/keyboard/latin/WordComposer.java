...
public final class WordComposer {
    ...
    private WordComposer(WordComposer other) {
        mEvents = other.mEvents != null ? new ArrayList<>(other.mEvents) : null;
    ...
    /**
     * Returns the count of digits in the composing word.
     */
    ...
    /**
     * Checks whether the current composing word is likely an accidental digit typo
     * (e.g. "th1s", "w9rk", "b4ck") rather than an intentional number or code (e.g. "2026", "a1").
     */
    public boolean isAccidentalDigitWord() {
        // Must have at least 1 digit, at least 3 total characters, and mostly letters (at most 1 digit for words < 6 chars, at most 2 for longer)
        if (mDigitsCount == 0 || mCodePointSize < 3) {
            return false;
        }
        if (mCodePointSize < 6) {
            return mDigitsCount == 1;
        }
        return mDigitsCount <= 2;
    }

    /**
     * Generates alternative word candidates by substituting accidental digit typo(s) with
     * nearby letter keys from the keyboard.
     * Checks both spatial proximity (touch coordinates or physical key distance)
     * and number-row / hint-label bindings (e.g. '1' -> 'q', '2' -> 'w' or hint '1' on key 'q').
     */
    public ArrayList<String> getNearbyLetterCandidatesForDigit(final helium314.keyboard.keyboard.Keyboard keyboard) {
        final ArrayList<String> candidates = new ArrayList<>();
        if (!isAccidentalDigitWord()) {
            return candidates;
        }
        final String typed = mTypedWordCache.toString();
        int digitIndex = -1;
        char digitChar = 0;
        for (int i = 0; i < typed.length(); i++) {
            final char c = typed.charAt(i);
        ...
        final ArrayList<Character> candidateLetters = new ArrayList<>();

        // 1. Direct standard number-row and leetspeak substitution mappings
        final char[] substitutions;
        switch (digitChar) {
            case '1': substitutions = new char[]{'q', 'i', 'l', 'w', 'a'}; break;
            case '2': substitutions = new char[]{'w', 'z', 'q', 'e', 's'}; break;
            case '3': substitutions = new char[]{'e', 'w', 'r', 'd'}; break;
            case '4': substitutions = new char[]{'r', 'a', 'e', 't', 'f'}; break;
            case '5': substitutions = new char[]{'t', 's', 'r', 'y', 'g'}; break;
            case '6': substitutions = new char[]{'y', 'g', 'b', 't', 'u', 'h'}; break;
            case '7': substitutions = new char[]{'u', 't', 'l', 'y', 'i', 'j'}; break;
            case '8': substitutions = new char[]{'i', 'b', 'u', 'o', 'k'}; break;
            case '9': substitutions = new char[]{'o', 'p', 'g', 'i', 'l'}; break;
            case '0': substitutions = new char[]{'p', 'o', 'l'}; break;
            default: substitutions = new char[0]; break;
        }
        for (final char sub : substitutions) {
            if (!candidateLetters.contains(sub)) {
                candidateLetters.add(sub);
            }
        }

        // 2. Keyboard inspection: hint labels and key proximity
        if (keyboard != null) {
            final String digitStr = String.valueOf(digitChar);
            final int digitCodePoint = (int) digitChar;
            helium314.keyboard.keyboard.Key digitKey = null;

            for (final helium314.keyboard.keyboard.Key key : keyboard.getSortedKeys()) {
                if (key.isSpacer()) continue;
                if (key.getCode() == digitCodePoint) {
                    digitKey = key;
                }
                // Check if this letter key has digitChar as its hint label or shifted letter hint
                final String hint = key.getHintLabel();
                if (hint != null && hint.equals(digitStr)) {
                    final int code = key.getCode();
                    if (Character.isLetter(code)) {
                        final char letter = Character.toLowerCase((char) code);
                        if (!candidateLetters.contains(letter)) {
            ...
            // Check touch coordinate from Event or center of digit key
            final Event ev = getEvent(digitIndex);
            final int touchX = (ev != null && ev.getX() != helium314.keyboard.latin.common.Constants.NOT_A_COORDINATE)
                    ? ev.getX()
                    : (digitKey != null ? digitKey.getX() + digitKey.getWidth() / 2 : helium314.keyboard.latin.common.Constants.NOT_A_COORDINATE);
            final int touchY = (ev != null && ev.getY() != helium314.keyboard.latin.common.Constants.NOT_A_COORDINATE)
                    ? ev.getY()
                    : (digitKey != null ? digitKey.getY() + digitKey.getHeight() / 2 : helium314.keyboard.latin.common.Constants.NOT_A_COORDINATE);

            if (touchX != helium314.keyboard.latin.common.Constants.NOT_A_COORDINATE && touchY != helium314.keyboard.latin.common.Constants.NOT_A_COORDINATE) {
                final ArrayList<helium314.keyboard.keyboard.Key> sortedByDist = new ArrayList<>();
                for (final helium314.keyboard.keyboard.Key key : keyboard.getSortedKeys()) {
                    if (key.isSpacer()) continue;
                    if (Character.isLetter(key.getCode())) {
                        sortedByDist.add(key);
                    }
                }
                Collections.sort(sortedByDist, (k1, k2) ->
                        Integer.compare(k1.squaredDistanceToEdge(touchX, touchY), k2.squaredDistanceToEdge(touchX, touchY)));

                for (int i = 0; i < Math.min(3, sortedByDist.size()); i++) {
                    final char letter = Character.toLowerCase((char) sortedByDist.get(i).getCode());
                    if (!candidateLetters.contains(letter)) {
                        candidateLetters.add(letter);
                    }
                }
        ...
        // Build substitution candidate words
        for (final char c : candidateLetters) {
            final StringBuilder sb = new StringBuilder(typed);
            sb.setCharAt(digitIndex, c);
    ...
    /**
     * @return unmodifiable view or list of input events.
     */
     ...
     * @return the event at the given index, or null if out of bounds or null list
     */
    ...
    static WordComposer getComposerForTest(String typedWord) {
        WordComposer composer = new WordComposer(false);
        composer.mTypedWordCache = typedWord;
        composer.mCodePointSize = typedWord.length();
        int digits = 0;
        for (int i = 0; i < typedWord.length(); i++) {
            if (Character.isDigit(typedWord.charAt(i))) {
                digits++;
            }
        }
        composer.mDigitsCount = digits;
        return composer;
    }
