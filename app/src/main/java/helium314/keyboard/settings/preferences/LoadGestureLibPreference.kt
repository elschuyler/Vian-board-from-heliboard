...
fun LoadGestureLibPreference(
    ...
    fun applyLibrary(file: File, checksum: String) {
        ...
        try {
            LogCatcher.markComponentActive("GestureEngine", "Native", "Installing user gesture library")
            ...
            if (success) {
                ...
                LogCatcher.i("LoadGestureLibPreference", "Native gesture library loaded successfully with checksum $checksum")
                ...
                onLibraryStateChanged()
                LogCatcher.e("LoadGestureLibPreference", "Failed to load user-supplied library (incompatible ABI or corrupted file)")
                Toast.makeText(context, "Failed to load library: incompatible ABI or corrupted file.", Toast.LENGTH_LONG).show()
            }
        } catch (e: Exception) {
            LogCatcher.e("LoadGestureLibPreference", "Error installing library: ${e.message}", e)
    ...
    val launcher = filePicker { uri ->
        try {
            LogCatcher.markComponentActive("GestureEngine", "Native", "Processing user-selected library file")
            ...
            cr.openInputStream(uri)?.use { input ->
                ...
                LogCatcher.e("LoadGestureLibPreference", "Failed to open input stream for selected library")
            ...
            if (checksum == null) {
                tempFile.delete()
                LogCatcher.e("LoadGestureLibPreference", "Could not compute checksum for chosen file")
            ...
            if (checksum.equals(expected, ignoreCase = true)) {
                ...
                LogCatcher.i("LoadGestureLibPreference", "Checksum mismatch: calculated=$checksum, expected=$expected")
                mismatchTempFile = tempFile
                mismatchChecksum = checksum
            }
        } catch (e: Exception) {
            LogCatcher.e("LoadGestureLibPreference", "Error processing selected file: ${e.message}", e)
        ...
        if (isInstalled && JniUtils.sHaveGestureLib) {
            ConfirmationDialog(
                ...
                onNeutral = {
                    ...
                    LogCatcher.markComponentActive("GestureEngine", "Native", "Deleting user gesture library")
                    JniUtils.deleteUserSuppliedLibrary(context)
                    context.protectedPrefs().edit().remove(Settings.PREF_LIBRARY_CHECKSUM).apply()
                    context.prefs().edit().remove(Settings.PREF_LIBRARY_CHECKSUM).apply()
                    isInstalled = false
