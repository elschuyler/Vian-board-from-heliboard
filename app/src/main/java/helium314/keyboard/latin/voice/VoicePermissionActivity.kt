...
object VoicePermissionBridge {
    private const val TAG = "VoicePermissionBridge"
    ...
    fun requestRecordAudioPermission(context: Context, callback: (Boolean) -> Unit) {
        if (hasRecordAudioPermission(context)) {
            LogCatcher.i(TAG, "RECORD_AUDIO permission already granted")
            callback(true)
            return
        }
        LogCatcher.i(TAG, "Requesting RECORD_AUDIO permission via VoicePermissionActivity")
